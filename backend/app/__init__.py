...
from app.services import StreamManager, SyntheticDataProvider, VendorDataProvider
...
def create_app(config_object=Config) -> Flask:
    ...
    live_configured = bool(
        app.config["TWELVEDATA_API_KEY"] and app.config["THETADATA_API_KEY"]
    )
    use_live = app.config["DATA_PROVIDER"] == "live" or (
        app.config["DATA_PROVIDER"] == "auto" and live_configured
    )
    provider = (
        VendorDataProvider(
        ...
        if use_live
        else SyntheticDataProvider()
    )
    app.extensions["streams"] = StreamManager(
        provider, repository, app.config["STREAM_INTERVAL"]
    )
